/*
  W212 CAN-E instrument-cluster ignition explorer
...
static void beginOkPulse() {
  wakePayload[5] = 0x02;
  okPulseActive = true;
  okPulseStart = millis();
  lastWakeTx = 0;
  Serial.println(F("OK pulse started: 0x045 byte 5 = 0x02"));
}
...
static void servicePeriodicFrames() {
  ...
  if (okPulseActive && (now - okPulseStart >= okPulseMs)) {
    ...
    okPulseActive = false;
    lastWakeTx = 0;
    Serial.println(F("OK pulse released"));
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println(F("W212 CAN-E ignition explorer"));
